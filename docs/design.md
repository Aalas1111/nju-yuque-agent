...
这个项目的核心判定全挂在「今天」上：**申请周期**、**可借日期 = 今天 +2 ~ +9 天**、
**每周六 00:00 归档**。而云服务器（阿里云 / 腾讯云 / 大多数 VPS）**默认就是 UTC**。

如果按服务器本地时间算，同一份代码搬到 UTC 机器上会：
...
> 为什么不写成 `timezone(timedelta(hours=8))`：那样说的是「偏移多少小时」，
> 而我们要表达的是「哪个**时区**」。中国 1991 年后没有夏令时，两者眼下完全等价，
> 但命名时区语义正确、也留了余地。`tzdata` 已加进依赖，所以 Windows 上
> （没有系统 IANA 库）行为与 Linux 一致。
...
后来任何一处顺手写的 `datetime.now()` 都会让那份逻辑重新跟回服务器时区，
而且**不会报错**——在开发机（+08:00）上永远看不出来，只在 UTC 服务器上错。
（用 AST 而不是文本匹配，是因为文档串里会引用 `datetime.now()` 作反例。）

`yqa doctor` 会把两件事都显示出来，方便上线时一眼确认：

```
│ 时区 │ Asia/Shanghai +08:00（写死，不受服务器设置影响；服务器当前 = UTC） │
```
...
> 根因是 `cli.py` 那一行**接线**没有任何测试守着：`poll_once` 被 `test_debounce.py`
> 测得再细，也保不住调用方漏传一个参数。现在 `tests/test_cli.py` 用
