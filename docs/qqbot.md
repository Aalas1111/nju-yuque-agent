...
| `/help` | `帮助` | 用户与管理员 | 列出命令。**按身份给**：管理员（含管理员群里的人）看到全部命令并按「只读 / 管理员专用」分组；普通用户只看到自己真正能用的那几条 |
...
`/help` 是**按身份生成**的：写操作对普通用户不可用，所以不在他的清单里；管理员（`inbound.admins`
里的人，或身在 `inbound.admin_groups` 群里的任何成员）拿到的才是完整清单，并单独标出管理员专用项。
